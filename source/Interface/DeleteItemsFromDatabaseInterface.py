...
from source.Interface.TextNameAndDescriptionC import TextNameAndDescriptionC
...
class ItemInformationInterface:
        ...
        with DB.atomic():
            for item in ItemsBlocked.select().where(ItemsBlocked.player == self.main_window.meta_player.name):
                ...
                self.list_items.append(
                    TextNameAndDescriptionC(item.name + " - " + ("blocked" if item.blocked else "unlocked"),
    ...
    def update(self):
        for i, prb in enumerate(self.list_items):
            prb.update(pg.mouse.get_pos(), self.screen, WIDTH // 2, 160 * i + 130 + self.shift)
