...
def calc_boss_shooting(boss: np.ndarray, enemy_bullets: np.ndarray, player: np.ndarray,
                       Id: np.ndarray, dt: np.float_) -> np.ndarray:
    quotient: np.float_
    amount: np.float_
    arange: np.ndarray
    data: np.ndarray
    ...
    if amount > 0:
        ...
        enemy_bullets[indices] = data

        return indices
    return np.array([])


def calc_boss_direction_(boss: np.ndarray, player: np.ndarray):
    angle: np.ndarray
...
def calc_boss_fight_start(player: np.ndarray, boss: np.ndarray, boss_type: np.ndarray, types: np.ndarray,
                          field: np.ndarray):
