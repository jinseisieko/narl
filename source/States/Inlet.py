...
class Inlet(InterfaceState):
    def begin(self):
        ...
        self.game.fps = self.inlet_interface.video.fps
        ...
        if event.type == pg.MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                if self.inlet_interface.buttons["LogIn"].update(mouse_pos):
                    self.game.meta_player.set(self.inlet_interface.text_input1.get(),
                                              self.inlet_interface.text_input2.get())
                    self.game.set_state(MainMenu(self.screen, self.game))
                if self.inlet_interface.buttons["SignIn"].update(mouse_pos):
                    self.game.meta_player.register(self.inlet_interface.text_input1.get(),
                                                   self.inlet_interface.text_input2.get())
                    self.game.set_state(MainMenu(self.screen, self.game))
